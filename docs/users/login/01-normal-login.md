...
| 节点        | 登录地址                                         | 端口 | 说明                             |
| ----------- | ----------------------------------------------| ---- | ------------------------------------- |
| node01      | `ssh.hpcer.dev`                                 | 22 | 计算集群的主登录节点，可以连接到其他节点。     |
| node02      | `x.hpcone.science`                             | 22 | 计算集群的登录节点，可以连接到其他节点。     |
| node[03-04] | `n03.hpcone.science`<br/>`n04.hpcone.science`     | 22 | 单独使用的节点。也可以由node01登录。       |
| node[13-16] | -                                             | 22 | Hadoop集群。                           |
| nodegpu     | `gpu.ssh.hpcer.dev`                             | 22 | GPU节点，单独使用。                      |
| nodestation | `station.ssh.hpcer.dev`                         | 22 | 工作站节点（英伟达 4090节点），单独使用。 |
| nodegdcu    | `dcu.ssh.hpcer.dev`                             | 22 | DCU 节点，单独使用。                     |

账号使用过程中有任何问题或申请开通账号请联系管理员: [储根深](mailto:genshenchu@gmail.com)。

## ssh登录集群
目前,该集群仅允许校内的网段登录,不对外网开放直接登录权限。
- ssh登录工具  
  - [ssh命令] 使用OS X、Linux或者window 10 1803以上版本内置的ssh命令
  - [MobaXterm](https://mobaxterm.mobatek.net)(windows 推荐);  
...
- ssh登录地址及端口  
主节点登录地址为 `ssh.hpcer.dev`，登录端口**22**。  
...
使用 MobaXterm 的用户使用以下方式生成公钥: [https://dora-cmon.github.io/posts/496913e0/](https://dora-cmon.github.io/posts/496913e0/)  
```
local$ ssh-keygen
...
```
如果是使用sftp工具上传公钥文件(例如上传mykey.pub到服务器~/.ssh/my_id_rsa.pub), 还需要登录服务器, 手动将公钥(Public Key)导入到**authorized_keys**文件中。
```bash
...
1. 登录集群 (如利用 PublicKey 登录或者通过管理员协助登录)，在用户目录执行如下命令以初始化:
  ...
  :::
2. 手机端前往应用商店下载[Google身份验证器](https://support.google.com/accounts/answer/1066447)或者[Microsoft Authenticator](https://www.microsoft.com/zh-cn/account/authenticator)应用。扫描创建账号时提供的二维码或者输入提供的密钥以创建动态验证码。
...
4. 以 xshell 为例，在xshell中选择文件->新建，在类别连接中填入主机名、端口号等内容，再在左侧的连接->用户身份认证，选择Keyboard-Interactive项。  
   用户也可直接采用 ssh 命令进行登录，登录过程中会要求输入密码和 Verification code（动态口令）。
<img alt="Xshell登录" src={useBaseUrl('users-assets/login.png')} />   

5. 在进行登录时(Xshell中从已创建的会话中登录), 会提示输入验证码, 输入手机端验证器提供的动态密码; 密码输入创建账号时提供的密码(如果有提示输入的话)。

## 使用sftp 客户端上传文件  
如果需要上传文件到集群的对应的用户目录，可使用支持sftp协议的工具上传。这里推荐使用MobaXterm工具和系统自带的 sftp 命令。
其中，MobaXterm 工具的具体配置方法见 https://mobaxterm.mobatek.net/documentation.html#4_7 ，其中主机名和端口号同ssh登录的设置，协议选择**sftp**。
